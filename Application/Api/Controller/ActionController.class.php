...
class ActionController extends Controller
{
    public function __construct()
    {
        ...
        //绑定Socket到端口
        $result = socket_bind($socket, $host, $port) or die("绑定端口失败\n");
        // 开始监听链接
        $result = socket_listen($socket, 3) or die("设置监听失败\n");
        // accept incoming connections
        // 另一个Socket来处理通信
        $spawn = socket_accept($socket) or die("接收失败\n");
        // 获得客户端的输入
        ...
        //处理客户端输入并返回结果
        $output = strrev($input) . "\n";
        socket_write($spawn, $output, strlen ($output)) or die("输出失败\n");
        // 关闭sockets
        socket_close($spawn);
        socket_close($socket);
    }
